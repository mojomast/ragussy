...
async function setup() {
  ...
  console.log('   5. Optional: Ragussy API key (if your target API requires x-api-key)\n');
  ...
  const defaultApiUrl = 'http://localhost:8000/api/ragussy';
  const apiUrl = await question(`RAG API URL [${defaultApiUrl}]: `);
  config.RAG_API_URL = apiUrl || defaultApiUrl;

  config.RAG_API_KEY = await question('Ragussy API Key (optional): ');
