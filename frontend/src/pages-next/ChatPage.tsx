...
export default function ChatPage() {
  ...
  useEffect(() => {
    void getSettings()
      ...
      .catch(() => undefined);
    void ragussyHealth().then(setHealth).catch(() => undefined);
  }, []);
  ...
  const ragussyStateText = useMemo(() => {
  ...
  const handleSend = async (event?: FormEvent) => {
    ...
    try {
      if (mode === 'direct') {
        const response = await ragussyDirectChat({ message: trimmed, conversationId });
        ...
        const response = await ragussyRagChat({
          message: trimmed,
          conversationId,
          includeCompare,
        });
      ...
      <aside className="xl:col-span-4 min-h-0 rounded-2xl border border-slate-800 bg-slate-900/85 p-4 text-slate-100 shadow-xl">
        ...
        <div className="space-y-4 overflow-auto pr-1">
          <section className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
            ...
            <p className="mt-2 text-xs text-slate-400">Ragussy status: {ragussyStateText}</p>
          </section>
          ...
          <section className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
            ...
            <div className="space-y-1 text-xs text-slate-300">
              <InfoRow label="Ragussy" value={ragussyStateText} />
