...
import { fetchIngestionProgress, ragussyHealth } from '../lib/nextApi';
...
export default function DashboardPage() {
  ...
  const load = async () => {
    try {
      const [data, ingest] = await Promise.all([ragussyHealth(), fetchIngestionProgress().catch(() => null)]);
      setHealth(data);
      setIngestion(ingest);
      setError('');
      ...
      console.error(err);
      setError('Unable to fetch health telemetry from Ragussy.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="mx-auto w-full max-w-7xl p-6 md:p-8">
      <header className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-400">Monitor your Ragussy stack, retrieval config, and runtime health.</p>
        </div>
      ...
      {!data.configured && !loading ? (
        <div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-400/30 bg-amber-400/10 p-4">
          ...
          <div>
            ...
            <p className="text-sm text-amber-100/80">Set `RAGUSSY_API_KEY` in Model Lab backend environment.</p>
          </div>
        </div>
      ) : null}
