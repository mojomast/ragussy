...
const RAGUSSY_SESSION_KEY = 'ragussy_session_id';
...
function getRagussySessionId(): string {
  if (typeof window === 'undefined') return '';
  let current = sessionStorage.getItem(RAGUSSY_SESSION_KEY);
  if (current) return current;
  current = typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function'
    ? crypto.randomUUID()
    : `sess-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  sessionStorage.setItem(RAGUSSY_SESSION_KEY, current);
  return current;
}

async function proxyRequest<T>(path: string, init?: RequestInit): Promise<T> {
  const headers = new Headers(init?.headers || {});
  headers.set('x-ragussy-session-id', getRagussySessionId());
  const res = await fetch(`/api/ragussy/proxy/${path}`, {
    ...init,
    headers,
  });
  if (!res.ok) {
...
export async function uploadDocuments(files: File[], autoIngest: boolean = false) {
  ...
  headers.set('x-ragussy-session-id', getRagussySessionId());
  const res = await fetch('/api/ragussy/proxy/documents/upload', {
    method: 'POST',
    headers,
    body: formData,
  });
...
export async function ragussyRagChat(payload: {
  message: string;
...
export async function ragussyDirectChat(payload: {
  message: string;
  conversationId?: string | null;
}) {
